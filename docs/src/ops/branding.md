...
- 修改后对所有用户立刻生效（浏览器缓存可能导致视觉更新稍有延迟，见下文）
...
5. 保存成功后，页面会自动刷新预览
   - 系统会从 `/branding.json` 拉取最新配置，并用 `updatedAt` 做 cache bust（在资源 URL 上拼接 `?v=...`）

## 缓存与刷新策略（为什么你有时“看起来没生效”）
...
系统在拉取 `/branding.json` 时有 5 秒超时保护；如果反向代理或缓存层导致请求挂起，页面会自动释放加载状态并允许继续操作。

## 相关链接

- [`TLS 与安全`](./tls.md)：生产环境建议启用 HTTPS，避免管理端资源被篡改
